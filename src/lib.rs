...
use self::winapi::shared::minwindef::{BOOL, DWORD, FALSE, LPVOID, TRUE};
...
use self::winapi::um::winnt::{
    JOBOBJECT_CPU_RATE_CONTROL_INFORMATION_u, JobObjectCpuRateControlInformation,
    JobObjectNetRateControlInformation, HANDLE, JOBOBJECT_CPU_RATE_CONTROL_INFORMATION,
    JOBOBJECT_NET_RATE_CONTROL_INFORMATION, JOB_OBJECT_ASSIGN_PROCESS,
...
pub fn assign_and_process_job(
    ...
    if do_skip {
        v_proc_handles.retain(|h_proc| {
            if is_proc_in_job(h_proc) == FALSE {
                println!("Process not in job...");
                return true;
            }
            println!("Process already in job");
            // if we're skipping, remove the handle from the vector
            // by returning false from the closure
            println!("Skipping process...");
            false
        });
    } else {
    ...
    println!(
        "Attempting to add {} processes to job...",
        v_proc_handles.len()
    );
...
fn ratelimit_net(h_job: HANDLE, net_rate: u32) -> Result<BOOL, Error> {
    unsafe {
        ...
        match SetInformationJobObject(
            ...
            jnrci_ptr,
            std::mem::size_of::<JOBOBJECT_NET_RATE_CONTROL_INFORMATION>() as u32,
        ) {
            TRUE => {
                println!("[Success!] Set max badnwidth (bytes/s) to {}%", net_rate);
                return Ok(TRUE);
            }
            _ => {
    ...
    unsafe {
        ...
        *jrci_cpu_ptr = cpu_pct_mul;
        ...
        match SetInformationJobObject(
            ...
            TRUE => {
                println!(
                    "[Success!] Set cpu percent control job information to {}%",
                    cpu_pct
                );
                return Ok(TRUE);
            }
